...
| `api/teach/propose/route.ts` | `GET` returns current pending proposal or null. `POST { days }` collects feedback, runs Lorenzo fresh, writes proposal sidecar with server-computed `warnings`. `DELETE` discards sidecar. To refine an existing proposal, use `propose/refine` — this route does not accept a `nudge` param. |

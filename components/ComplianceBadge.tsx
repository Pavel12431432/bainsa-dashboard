  ...
  return (
    <div className="flex gap-1 flex-wrap justify-end">
      {failures.map(({ key, label }) => (
        ...
        >
          ✕ {label.toUpperCase()}
          <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 px-2 py-1 rounded bg-border border border-border-mid text-[0.6rem] text-brand-white font-normal tracking-normal whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
            {result[key].detail}
          </span>
        </span>
      ))}
